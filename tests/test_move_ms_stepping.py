...
def test_click_no_move_ms_single_shot(server, monkeypatch):
    """Default ``move_ms=0`` (omitted) emits exactly ONE mouse_move
    HID report, same as pre-v0.2.8."""
    monkeypatch.setattr(
        "clawtouch_mcp.server.get_cursor_position",
        lambda: (100, 100),
    )
...
def test_click_with_move_ms_emits_multiple_reports(server, monkeypatch):
    """200 ms / 10 ms per step → 20 reports. Sum of step deltas
    must equal requested (400, 300)."""
    monkeypatch.setattr(
        "clawtouch_mcp.server.get_cursor_position",
        lambda: (100, 100),
    )
    ...
    # Sum of all per-step deltas == total requested move
    sum_dx = sum(m[0] for m in moves)
    sum_dy = sum(m[1] for m in moves)
    assert sum_dx == 400
    assert sum_dy == 300
    # 20 mouse_moves + 1 mouse_click = 21 total bridge calls
...
def test_move_ms_minimum_4_steps(server, monkeypatch):
...
def test_hover_move_ms_and_duration_ms_independent(server, monkeypatch):
    ...
    moves = _move_calls(server.bridge)
    assert len(moves) == 10  # stepped
    assert result["steps"] == 10
    assert sum(m[0] for m in moves) == 200
    assert sum(m[1] for m in moves) == 100


def test_hover_move_ms_zero_still_idles(server, monkeypatch):
    """move_ms=0 (default) → teleport, then idle for duration_ms."""
    monkeypatch.setattr(
        "clawtouch_mcp.server.get_cursor_position",
        lambda: (0, 0),
    )
    ...
    moves = _move_calls(server.bridge)
    assert len(moves) == 1  # teleport
    assert moves[0] == (200, 100)


# ── safety cap ──────────────────────────────────────────────────

def test_move_ms_clamped_to_max(server, monkeypatch):
...
def test_stepped_zero_distance_no_report(server, monkeypatch):
    """Stepping a (0, 0) move shouldn't emit any HID reports —
    every per-step delta rounds to 0."""
    monkeypatch.setattr(
        "clawtouch_mcp.server.get_cursor_position",
        lambda: (500, 500),
    )
