...
pip install 'clawtouch-mcp[screenshot]'       # + mss + Pillow (JPEG / Retina 缩放)
pip install 'clawtouch-mcp[screenshot-min]'   # 只 mss, 无原生依赖; 库验证宿主可用
```
...
**`hid.screenshot` 报 `ImportError: dlopen(... _imaging ...) code signature
... different Team IDs`?**
你的宿主 Python 启用了 hardened runtime 的*库验证*(library validation),只允许
加载与宿主同 Team ID 签名的原生扩展 —— 于是拦下了 Pillow 的 `_imaging`(一些
捆绑 / 启动器 Python 会这样)。server 会**自动检测**并降级到无原生扩展的
**`mss-png`** 后端,返回 PNG 截图并在 metadata 里附一句说明 —— 你无需做任何事。
mss 本身是纯 Python(ctypes → CoreGraphics,平台框架,豁免),照常加载;只有
Pillow 的编译扩展被挡。要强制走它,传 `--screenshot-backend mss-png` 或装精简的
`pip install 'clawtouch-mcp[screenshot-min]'`(只 mss)。要继续用 Pillow(JPEG +
更高质量缩放),从未启用库验证的 Python 跑 `clawtouch-mcp`,或给宿主 Python 加
`com.apple.security.cs.disable-library-validation` entitlement。无论哪种,截图的
`backend` 字段和 `scale_x`/`scale_y` 会告诉 agent 拿到的是什么 —— 永远把点击
坐标除以 `scale_x`/`scale_y`。
