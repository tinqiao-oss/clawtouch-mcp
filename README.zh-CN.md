...
<p align="center">
  <img src="docs/assets/hero.svg" alt="clawtouch-mcp 数据流: LLM agent 通过 MCP stdio JSON-RPC 调用 clawtouch-mcp, 后者通过 USB-CDC 发送带帧字节到 Raspberry Pi Pico 2 (跑 ClawTouch HID 固件), Pico 再输出标准 USB HID 报告到目标操作系统 (Windows / macOS / Linux)." width="900">
</p>
...
[INFO] 注册 9 个 HID 工具 + 2 个 device 工具, 监听 stdio
...
> 🎥 同样流程对真实 ClawTouch 硬件的屏幕录制 GIF 后续会补到这里。
> Pico 在维护者手里; 上面这段是真实 Pico 2 跑 `--log-level INFO`
> 输出 (serial 已随机化)。
...
flowchart LR
    A["<b>LLM Agent</b><br/><sub>Claude Desktop / Cline /<br/>Cursor / OpenClaw / Hermes / ...</sub>"]
        -->|"stdio<br/>JSON-RPC<br/>MCP 2024-11-05"| B["<b>clawtouch-mcp</b><br/><sub><i>本仓库</i><br/>MCP server + 9 HID 工具</sub>"]
