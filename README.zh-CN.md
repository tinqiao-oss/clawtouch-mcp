...
OS 级合成输入需要在目标机上跑 agent 进程,而且行为可能跟真实输入有差异 ——
在锁机 kiosk、嵌入式测试台架、辅助技术兼容性场景下都会出问题。HID 输入走
系统标准 HID 驱动栈,**跟外接物理键盘鼠标走完全相同的输入路径**,目标机
零安装。这点对辅助技术兼容性测试、kiosk 自动化、离线测试台架、跨设备 RPA
等场景至关重要。
