...
如果你想干的是上述两类事,这不是合适的工具。涉及合规边界另见下文
「可接受用途」。
...
## 自主性与安全 —— 接入本工具的 agent 能做什么

> 接入自主 agent 前请先读本节。上面的运行时限制是防误点/防洪泛的护栏,
> **不是**针对失控 agent 的安全边界。

`clawtouch-mcp` 把你的 agent 的工具调用变成**真实的 USB HID 输入** —— 走的是
跟外接键鼠相同的驱动路径, 因此在输入层面操作系统会把它当作物理设备输入来处
理。正因为它是真正的 HID 设备、而非合成注入的输入, 那些正当用途才成立(受限
kiosk、兼容性测试、无障碍辅助、跨机 RPA)。同一特性也带来一个你需要为之做准
备的对称风险:

**接入这里的自主 agent, 实际上对宿主机的触及范围跟坐在键盘前的真人相当。**通
过普通的 HID 输入, 它可以打开任意程序、在终端里执行命令、启动程序、安装或卸载
软件、读取/移动/删除文件。而且由于输入是以普通 HID 形式送达, 交互式确认弹窗本
身并不能可靠地拦住它 —— 应把任何同意或授权类弹窗都视为 agent 可能会去操作的对
象。`clawtouch-mcp` **不会**检查 agent 发来内容的意图或文本, 它只是把每一次调用
忠实转发给硬件。

这一切可能**并非出于你的本意**, 因为决定做什么的是 agent, 而不是本 server。常
见触发因素:

- **提示词注入 (prompt injection)。**agent 从屏幕、网页或图片里读到的不可信文本
  中, 可能嵌有覆盖你指令的对抗性指示, 把 agent 引向你从未要求的动作。读屏 /
  RPA 类工作流是这种风险会出现的场景之一。
- **模型出错。**模型可能只是误解了任务, 在错误的窗口、错误的文件或错误的按钮
  上动作。
- **过宽的自主权。**任务越开放、检查点越少, 一旦发生上述情况, 爆炸半径就越大。

这描述的是自主 agent 的一种**非预期失效模式**, 并非受支持的用法 —— 故意用 HID
输入去破坏某个系统的安全或风控措施, 属于下文**可接受用途**的范围之外。它也与
**内容生成**(标识)段、以及 [`SECURITY.md`](SECURITY.md) 里讲的软件漏洞是不同
的关注点; 那些都不覆盖 agent **违背你本意**行动这种情形。MIT 协议的「AS IS / 不
提供任何担保」是免责条款, 不是 informed-risk 披露; 安全地运行一个自主 agent 的
责任在你这位部署者身上。本节仅供参考, 不修改、限缩或扩张 MIT 协议, 不构成任何
担保或注意义务, 亦不将责任转移给亭桥; MIT 的免责与责任限制条款继续完整适用。

### 部署者缓解措施

把驱动 HID 输入的 agent, 当成你交给了一位能力强但尚不完全可信的操作员一双能动
手的真手。建议:

- **用可随时重置/重装的专用机器(或虚拟机 / 容器)** —— 不要用你的主力电脑。
  Local mode 把 agent 和目标放在同一台 PC: 那是方便的常见情形, 也是爆炸半径最
  大的情形; 条件允许时优先用单独的机器。
- **以最小权限的 OS 账户运行; 不要用 administrator / root 跑目标会话或 agent。**
  agent 会继承该账户能做的一切。
- **把敏感数据和已登录账户挪出目标机** —— 不保存密码、不保留已认证会话、不把凭
  据写进 prompt —— 这样一旦发生注入或误操作, 它能触及的东西更少。
- **保留 human-in-the-loop。**在有现实后果或不可逆的动作前要求真人显式确认 ——
  安装/删除软件、发送消息或邮件、金融交易、同意条款。不要让 agent 在开放式任务
  上无人值守长跑。
- **隔离网络**(例如用域名白名单), 减少 agent 接触恶意或带注入内容的机会。
- **把 agent 从屏幕或网页读到的一切都当作不可信输入**, 并将其与敏感数据和动作隔
  离, 这样即便注入得手, 能造成的损害也更小。
- **随手备好急停。**`hid.release_all` 可从 agent 一侧释放所有按下的键和鼠标按钮;
  物理拔掉 Pico 的 USB 线是最可靠的急停, 能彻底切断 agent 的输入通路。
...
2026-01) 是硬件预算 (< $30: RP2040 + HDMI→USB + CH340 串口桥) 和设计意图
上最接近的学术同行; 配套是 Python 库, 不带 MCP server。

只要在本机做合成输入、且应用不挑输入来源, 上面那些纯软件 MCP 更省事,
还省了买硬件。ClawTouch 的不可替代之处是那条真实硬件 HID 通路: OS 看到
的是真实物理键鼠 —— 本机模式下, 这正是无障碍 / 兼容性测试 / 挑剔合成
输入的应用用得上的点 (输入侧还零驱动); 在此之上, 跨机模式还能控制装不了
agent、或必须物理隔离的目标机, 这是纯软件方案根本做不到的。
